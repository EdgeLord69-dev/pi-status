...
- Phases 1–7 are complete. Phase 1 owns one global atomic `saveConfig()` for the extension-owned config file, Phase 2 owns `StatusLineZones` and cloning/normalization, Phase 3 and Phase 7 supply all telemetry/activity segment IDs below, Phase 4 supplies the single command router, and Phase 6 adds the notification field that preset application preserves.
...
- The existing editor save closure remains the only config writer and preserves `completionNotifications` while replacing `zones` only.
...
- [ ] Add failing index/config integration tests for one global direct-file write replacing all four zones, legacy `segments` absent after save, `completionNotifications` preserved, and failed writes preserving the previous live layout.
- [ ] Run:
...
- [ ] Explain that preview/confirmation is mandatory, the existing global extension-owned config file applies, optional unavailable items may render blank, and only zone selection/order changes.
...
- [ ] Manually cancel selection, select each preset, inspect all four preview rows, reject one confirmation, confirm one in the global extension-owned config file, restart Pi, and verify exact persistence and two-row rendering.
