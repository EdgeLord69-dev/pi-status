...
Refactor pi-status so completion notifications travel through the active terminal environment instead of operating-system notification APIs, and so the sidebar derives every color from Pi's current theme instead of the fixed Midnight palette.
