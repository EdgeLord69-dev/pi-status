...
**Tech Stack:** TypeScript 6, current locked development baselines `@earendil-works/pi-coding-agent@0.82.0` and `@earendil-works/pi-tui@0.82.1` from the existing caret ranges, wildcard runtime peer dependencies for both packages, Node `>=24.15.0`, Vitest 4, Biome 2, pnpm 11, Node standard library, read-only Git subprocesses.
...
2. Extend `StatusLineSegmentId` additively. Phase 2 replaces runtime `PiStatusConfig.segments` with four zones while accepting direct legacy arrays only as load-time migration input; later phases must preserve that migration.
...
9. Keep the current development dependency ranges and lockfile unchanged, use the locked Pi agent/TUI versions above, keep published runtime peer dependency ranges as `"*"`, and preserve `engines.node` as `">=24.15.0"`.
