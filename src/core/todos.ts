...
const VALID_STATUSES = new Set<NormalizedTodo["status"]>(["pending", "in_progress", "completed"]);
const MAX_TODO_ITEMS = 2048;

const record = (value: unknown): Record<string, unknown> | undefined =>
  typeof value === "object" && value !== null ? (value as Record<string, unknown>) : undefined;

function normalizeTodo(value: unknown, oldShape: boolean): NormalizedTodo | undefined {
...
export function parseTodoDetails(details: unknown): NormalizedTodo[] | undefined {
  const source = record(details);
  if (!source) return undefined;
  const oldShape = Array.isArray(source.todos);
  const items = oldShape ? source.todos : Array.isArray(source.tasks) ? source.tasks : undefined;
...
export function reconstructTodos(branch: readonly unknown[]): NormalizedTodo[] {
  for (let index = branch.length - 1; index >= 0; index -= 1) {
    ...
    if (message?.role !== "toolResult" || message.toolName !== "todo" || message.isError === true) {
      continue;
    }
